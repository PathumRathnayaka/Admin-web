    ...
    <label className="block" htmlFor={id}>
      ...
      <div className="relative">
        <input
          ...
          className={`h-12 w-full rounded-lg border border-slate-200 bg-white px-4 ${isPassword ? 'pr-12' : ''} text-sm text-slate-950 outline-none transition placeholder:text-slate-400 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 dark:border-slate-700 dark:bg-slate-900 dark:text-white dark:placeholder:text-slate-500 ${className}`}
          {...props}
        />
        {isPassword && (
          <button
